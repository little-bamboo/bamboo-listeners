...
class CruisecriticAuthSpider(scrapy.Spider):
    ...
    def auth_login(self, response):
        ...
        if 'authentication failed' in response.body:
            ...
            while num < 200: #1583505:
                url = base_profile_url + str(num)
                num += 1
                time.sleep(2)
    ...
    def obtain_profiles_list(self, response):
        ...
        profiles_list = response.xpath('//a[contains(@href, "member.php?u=")]/@href').extract()
    ...
    def obtain_profiles_info(self, response):
    ...
    def obtain_user_posts(self, response):
        ...
        post_list = response.xpath('//div[@class="box1"]')

        for item in post_list:
            try:
                forum_name = item.xpath('.//div[contains(@class, "pull-right")]/a//text()').extract()[0].strip()
                post_datetime_raw = item.xpath('.//div[contains(@class, "box-footer")]/div//text()').extract()[0].strip()
                post_datetime = datetime.strptime(post_datetime_raw, '%b %d, %Y, %I:%M %p')
                post_id_raw = item.xpath('.//div[contains(@class, "box-content")]/a/@href').extract()[0]
                post_id = post_id_raw.split('=')[1].split('#')[0]
                post_title = item.xpath('.//div[contains(@class, "box-content")]/a/text()').extract()[0].strip()
                post_body_list = item.xpath('.//div[contains(@class, "box-content")]//text()').extract()[2:]
                post_body = ' '.join(post_body_list).strip()
                post_thread_title = item.xpath('.//div[contains(@class, "box-header")]/div[2]/a//text()').extract()[0].strip()
                post_thread_id = item.xpath('.//div[contains(@class, "box-header")]/div[2]/a/@href').extract()[0].split('=')[1].strip()
                post_user_id = item.xpath('.//div[contains(@class, "box-header")]/div[3]/a/@href').extract()[0].split('=')[1].strip()
                post_url = item.xpath('.//div[contains(@class, "box-content")]/a/@href').extract()[0].strip()
                ...
                post['thread_id'] = post_thread_id
                post['user_id'] = post_user_id
                post['thread_title'] = post_thread_title
